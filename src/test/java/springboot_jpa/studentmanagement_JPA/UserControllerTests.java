...
public class UserControllerTests {
	...
	public void usersearchTest() throws Exception {
		...
		userlist.add(user1);
		when(userService.getAllUsers()).thenReturn(userlist);
		this.mockMvc.perform(post("/searchuser").param("userID", "").param("userName", "")).andExpect(status().isOk())
				.andExpect(view().name("USR003")).andExpect(model().attributeExists("userlist"));
		when(userService.getUserByName("John")).thenReturn(userlist);
		this.mockMvc.perform(post("/searchuser").param("userID", "").param("userName", "John"))
				.andExpect(status().isOk()).andExpect(view().name("USR003"))
				.andExpect(model().attributeExists("userlist"));
		when(userService.getUserById("USR001")).thenReturn(userlist);
		this.mockMvc.perform(post("/searchuser").param("userID", "USR001").param("userName", ""))
				.andExpect(status().isOk()).andExpect(view().name("USR003"))
				.andExpect(model().attributeExists("userlist"));
	...
	public void setupaddUserTest() throws Exception {
		this.mockMvc.perform(get("/user/add-user")).andExpect(status().isOk()).andExpect(view().name("USR001"))
	...
	public void addUserPasswordFalseTest() throws Exception {
		...
		this.mockMvc.perform(post("/adduser").flashAttr("user", user1)).andExpect(status().isOk())
				.andExpect(view().name("USR001")).andExpect(model().attributeExists("error"));
	}
	...
	public void addUserPasswordTrueTest() throws Exception {
		...
		userService.addUser(user1);
		this.mockMvc.perform(post("/adduser").flashAttr("user", user1)).andExpect(status().isOk())
				.andExpect(view().name("USR001")).andExpect(model().attributeExists("success"));
	}

	@Test
	public void setupUpdateUserTest() throws Exception {
		when(userService.getUserbyUserId("USR001")).thenReturn(new User());
		this.mockMvc.perform(get("/setupUpdateUser").param("userId", "USR001")).andExpect(status().isOk())
				.andExpect(view().name("USR002")).andExpect(model().attributeExists("User"));
	}
	...
	// testing updateuser method when validated pass
	@Test
	public void updateUserPasswordFalseTest() throws Exception {
		...
		this.mockMvc.perform(post("/updateuser").flashAttr("User", user1)).andExpect(status().isOk())
				.andExpect(view().name("USR002")).andExpect(model().attributeExists("error"));
	}
	...
	public void updateUserPasswordTrueTest() throws Exception {
		...
		userService.updateUser(user1);
		this.mockMvc.perform(post("/updateuser").flashAttr("User", user1)).andExpect(status().isOk())
				.andExpect(view().name("USR002")).andExpect(model().attributeExists("success"));
	}
